...
const imageSkewAngle01 = function (
  ...
  try {
    ...
    OpenCVcvtColor(mat, OpenCVCOLOR_BGR2GRAY());

    // apply gaussian blue
    OpenCVgaussianBlur(mat, 9);

    // apply threshold
    OpenCVThreshold(blur, 0, 255, OpenCVTHRESH_BINARY_INV() + OpenCVTHRESH_OTSU());

    // Apply dilation to merge text into meaningful lines/paragraphs.
    // Use larger kernel on X axis to merge characters into single line, cancelling out any spaces.
    // But use smaller kernel on Y axis to separate between different blocks of text
    OpenCVgetStructuringElement(OpenCVMORPH_RECT(), 30, 5);
    const dilateIterations = 5;
    const kernel = null;
    OpenCVDilate(mat, kernel, dilateIterations);

    // Find all contours
    OpenCVfindContours(mat);

    // Find the largest contour and surround in min area box
    const largestContour = mat[0];
