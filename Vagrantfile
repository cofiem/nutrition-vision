...
Vagrant.configure("2") do |config|
  ...
  rsync_exclude = %w[.vagrant/ .vscode/ .idea/ .angular/ src/ node_modules/]
  ...
  new_apt_url = "http://mirror.aarnet.edu.au/pub/ubuntu/archive"
  ...
  config.vm.provision "install_ansible", type: "shell", inline: <<-SHELL
    ...
    #{ansible_venv_dir}/bin/ansible-galaxy collection install --upgrade community.general ansible.posix
  SHELL
  ...
  config.vm.provision "run_ansible", type: "ansible_local" do |ans|
    ...
    ans.config_file = "#{work_source_dir}/dev/ansible/ansible.cfg"
    ans.playbook = "#{work_source_dir}/dev/ansible/playbook.yml"
    ans.extra_vars = {
